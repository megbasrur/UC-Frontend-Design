...
/*    const validateMobile=(mobileno)=>{
    ...
    const handleSubmit = async (e) => {
       ...
       /* if (!validateName(name)) {
        ...
        try {
            const response = await axios.post('https://uc-fd-auth-backend.onrender.com/user/login', {email, password},
    ...
    return (
        <Box className="bg-gray-800 flex flex-col justify-center items-center min-h-screen">
  <img src={Logo} className="w-32 mb-4" />
        <Container maxWidth="xs" sx={{ mt:4, mb:4, paddingBottom:5, borderRadius:10 ,boxShadow: '2px 2px #9ea1a3 inset',display:'flex', flexDirection:'column', justifyContent:'center', alignItems:'center',backgroundImage:'radial-gradient(#d8d8da,#fefefe)', backdropFilter:'blur(5px)'}}>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <Button type="submit" variant="contained" color="primary" sx={{marginTop:4, marginLeft:20}}>
                    Log In
                </Button>
            </form>
            {successMessage&&(
                <Typography variant="body1" color="success.main" sx={{marginTop:3}}>
                    {successMessage}
                </Typography>
            )}
            {errorMessage&&(
                 <Typography variant="body1" color="error.main" sx={{marginTop:3}}>
                 {errorMessage}
             </Typography>
            )}
             <Typography variant="body2" sx={{ marginTop: 4 }}>
                ...
                <Typography variant="body2" sx={{ marginTop: 2 }} className='text-blue-600'>
                   Click here to <Link to="/home">Browse as Guest</Link>
                </Typography>
        </Container>
        </Box>
    );
